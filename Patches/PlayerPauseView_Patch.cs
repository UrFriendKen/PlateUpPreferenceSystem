...
namespace PreferenceSystem.Patches
{
    ...
    internal class PlayerPauseViewPatch
    {
        ...
        {
            FieldInfo field = ReflectionUtils.GetField<LocalMenuView<PauseMenuAction>>("Panel");
            PanelElement panelElement = (PanelElement)field.GetValue(__instance);
            FieldInfo field2 = ReflectionUtils.GetField<LocalMenuView<PauseMenuAction>>("ModuleList");
            ModuleList module_list = (ModuleList)field2.GetValue(__instance);
            panelElement.gameObject.SetActive(value: false);
            MethodInfo method = ReflectionUtils.GetMethod<LocalMenuView<PauseMenuAction>>("AddMenu");
